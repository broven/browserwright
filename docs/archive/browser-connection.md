...
> **历史说明（2026-05）**：本文写于 `autoconnect` backend 还在 daemon 内时。如今 `autoconnect` 已经从 `browserwright-daemon` 移除（Chrome 144+ popup-accumulation hazard 没法靠"开发者自律"防住）。要驱动用户日常 Chrome，请启动单全局 daemon：`browserwright-daemon serve`，装配套未打包扩展，并用 `browserwright session new --backend=extension` 为具体任务选择 extension session。它靠 `chrome.debugger` 中继 CDP，零 popup、零横幅。下文中对 AutoConnect 的描述属于 Chrome 自身这条能力的 field notes，仍然准确，但 daemon 不再走这条路。
