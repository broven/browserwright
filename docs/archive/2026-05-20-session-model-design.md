...
- `browserwright session new [--name <label>]` → 返回**短自增 id**(从 `1` 起,省 token)。
...
- **取消 default session**:不带 id → 响亮报错 `"no session; run browserwright
  session new first"`,无静默回退。直接干掉 `mode_b_client.py:39` 的 `_DEFAULT_NAME
  = os.environ.get("BD_NAME", "default")` 这个 import 期冻结的事故源。
- `browserwright session end --session <id>` → 按 §1.3 ownership 规则清理。
...
- 位置:`~/.browserwright/sessions/`,**带文件锁**。
