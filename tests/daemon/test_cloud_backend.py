...
Three angles:
  ...
  3. fallback chain: `cloud` is NOT in the auto chain — only triggered via
     ...
     must never silently leak through `browserwright-daemon url`.
"""
...
from browserwright.daemon.resolver import resolve as resolver_resolve
...
async def test_resolve_http_endpoint_walks_discovery_with_header(monkeypatch):
    monkeypatch.setattr(
        "browserwright.daemon.backends.cloud.httpx.AsyncClient",
...
async def test_cloud_backend_not_in_auto_fallback_chain(monkeypatch):
    """Critical safety invariant: `browserwright-daemon url` with no explicit
    ...
    monkeypatch.setattr(
        "browserwright.daemon.backends.env.EnvBackend.resolve", boom)
    monkeypatch.setattr(
        "browserwright.daemon.backends.rdp.RdpBackend.resolve", boom)
    ...
    monkeypatch.setattr(
        "browserwright.daemon.backends.cloud.CloudBackend.resolve", cloud_boom)
...
async def test_probe_does_not_open_network_connection_to_cloud(monkeypatch):
    ...
    monkeypatch.setattr(
        "browserwright.daemon.backends.cloud.httpx.AsyncClient", _Boom)
