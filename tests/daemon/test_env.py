...
from browserwright.daemon.backends.env import EnvBackend
...
async def test_bd_cdp_url_resolves_via_json_version(monkeypatch):
    ...
    monkeypatch.setattr("browserwright.daemon.backends.env.httpx.AsyncClient", FakeClient)
...
def test_extension_port_default_is_19989():
    ...
    from browserwright.daemon.server.relay import DEFAULT_RELAY_PORT
    cfg = load(env={})
...
def test_bug11_allow_default_profile_env_truthy_variants_unlock(env_value, monkeypatch):
    ...
    from browserwright.daemon.launch_chrome import _truthy_env
...
def test_bug14_default_backend_garbage_string_raises_at_resolve_time(tmp_path):
    ...
    import asyncio
    from browserwright.daemon.errors import UserError
    from browserwright.daemon.resolver import resolve
