...
1. **Surface contract (offline, always runs).** ``snapshot`` and
   ``describe_page`` are in ``browserwright.EXPORTS``, importable from the
...
def test_primitives_in_exports():
    ...
    assert "snapshot" in browserwright.EXPORTS
    assert "describe_page" in browserwright.EXPORTS


def test_primitives_importable_from_namespace():
    from browserwright import describe_page, snapshot  # noqa: F401
...
def test_describe_page_accepts_viewport_only_kwarg():
    ...
    from browserwright import describe_page
...
def live_session(tmp_path_factory):
    ...
    from browserwright.session import with_session
...
def test_describe_page_viewport_only_excludes_offscreen(injected):
    ...
    from browserwright import describe_page, js
...
def test_snapshot_returns_interactive_nodes_with_center_xy(injected):
    ...
    from browserwright import snapshot
