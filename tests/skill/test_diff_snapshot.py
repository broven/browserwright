...
1. **Surface contract (offline, always runs).** ``diff_snapshot`` is in
   ``browserwright.EXPORTS``, importable from the top-level namespace, and
...
def test_diff_snapshot_in_exports():
    ...
    assert "diff_snapshot" in browserwright.EXPORTS


def test_diff_snapshot_importable_from_namespace():
    from browserwright import diff_snapshot  # noqa: F401
...
def test_diff_snapshot_signature_before_after():
    ...
    from browserwright import diff_snapshot
...
def test_diff_snapshot_buckets_exact_mutations(live_session):
    from browserwright import diff_snapshot, js, snapshot
