...
push (the ``subprocess.run`` to ``browserwright-daemon``) and the reload + screenshot
...
def _patch_reload_and_shot(monkeypatch, reload_exc=None):
    ...
    # from browserwright.api — patch on that module).
    import browserwright.api as api
...
def test_without_verify_only_push_runs(monkeypatch, capsys):
    ...
    assert calls["push_argv"][:2] == ["browserwright-daemon", "userscript"]
...
def test_with_verify_reloads_then_screenshots_after_push(monkeypatch, capsys):
    ...
    assert rc == 0
    # The --verify flag is a browserwright concern; it must not be forwarded.
    assert "--verify" not in calls["push_argv"]
