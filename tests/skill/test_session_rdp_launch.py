...
def test_create_launches_chrome_and_serve(tmp_bs_home, spawned):
    ...
    assert any("serve" in c and "browserwright-daemon-s" + sid in c for c in cmds)
    record = reg.get(sid)
    assert record["owner"] == "create"
    assert record["daemon_endpoint"] == f"browserwright-daemon-s{sid}"
...
def test_attach_points_daemon_at_port(tmp_bs_home, spawned):
    ...
    assert len(serve) == 1
    assert "9222" in serve[0] and f"browserwright-daemon-s{sid}" in serve[0]
...
def test_close_browser_stops_daemon_and_kills_chrome(tmp_bs_home, spawned):
    ...
    assert any("stop" in c and "browserwright-daemon-s1" in c for c in spawned["run"])
