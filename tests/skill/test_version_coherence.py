...
*installed* `browserwright-daemon` package, the connect/ensure path stops + respawns
...
def _client_with_versions(monkeypatch, running, installed, *, backend="extension"):
    ...
    # The "running is None but alive (legacy daemon)" branch consults is_alive();
    # stub it so the test is hermetic and doesn't depend on a live daemon socket
    # happening to be reachable on the dev box.
...
def test_version_probe_reads_status_json(monkeypatch):
    ...
    import browserwright.mode_b_client as m
...
def test_installed_version_reads_version_subcommand(monkeypatch):
    ...
    class _Proc:
        returncode = 0
        stdout = "browserwright-daemon 0.5.3\n"
        stderr = ""
