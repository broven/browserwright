...
def test_global_set_preference_writes(tmp_bs_home, fresh_modules):
    from browserwright.memory import global_memory, read_daemon_preferred_backend
...
def test_site_bootstrap_lazy_create(tmp_bs_home, fresh_modules):
    from browserwright.memory import bootstrap_site, site_dir
...
def test_redaction_blocks_high_entropy(tmp_bs_home, fresh_modules):
    from browserwright.memory.site_mem import RedactionRejected, SiteMemory
