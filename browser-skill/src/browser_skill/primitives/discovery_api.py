...
``browser_skill.discovery`` and ``browser_skill.task_runner`` so an agent
...
def load_site_skill(site: str, name: Optional[str] = None) -> Any:
    ...
    mod_name = f"browser_skill_task_{host_stem(site).replace('.', '_')}_{name}"
...
def run_task(site: str, name: Optional[str] = None, **kwargs) -> Any:
    ...
    Re-exports ``browser_skill.task_runner.run_task`` so agents calling
