...
_OPTIONS: list[Tuple[str, str, str, str]] = [
    ("1", "rdp",
     "隔离 profile (rdp + browser-daemon launch-chrome)  [Recommended]",
...
def chrome_extension_path() -> Optional[str]:
    ...
    Resolution order:
      ...
      2. ``browser-daemon extension-path --json`` subprocess (the v0.4 daemon
         exposes the resource path; cheap one-shot, no ws side effects).
      3. Best-effort walk from ``which browser-daemon`` — many dev checkouts
    ...
    bin_path = shutil.which("browser-daemon")
    if bin_path:
...
def _daemon_config_path() -> Path:
    """Return ``~/.config/browser-daemon/config.toml`` (XDG-aware).
    ...
    return Path(base) / "browser-daemon" / "config.toml"
...
def _collect_cloud_fields(provider: str, auth_kind: str,
    ...
    prompt defaults so re-running ``browser-skill install`` doesn't
...
def run() -> int:
    ...
    if not _yesno("Persist this preference to ~/.browser-skill/global.md?"):
    ...
    if choice == "1":
        ...
        if ext_dir:
            ...
            print("         (Hint: `browser-daemon extension-path --json` prints it.)")
        print("  2. Start the daemon in extension-relay mode:")
        print("       browser-daemon serve --backend extension"
        ...
        print(f"       browser-daemon serve --backend cloud --provider {provider}"
              f" --name {os.environ.get('BD_NAME', 'default')}")
        print("       (daemon-impl-2 may also expose `browser-daemon set-credentials`")
