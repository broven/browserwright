    ...
    def doctor_checks(self) -> dict:
        ...
        def add(name, status, message, fix=""):
            ...
            if status == "fail" and not (fix and fix.strip()):
                fix = "run `browser-skill doctor` and address the first failing check"
            checks.append({"name": name, "status": status,
                           "message": message, "fix": fix})

        # 1. daemon reachable (did `browser-daemon doctor` actually answer?)
        if info.get("skill_synthetic"):
            add(
                "daemon",
                "fail",
                info.get("error") or "browser-daemon did not respond",
                "install/start the daemon: ensure `browser-daemon` is on PATH "
                "then `browser-daemon serve --backend <extension|rdp>`",
            )
        else:
            add("daemon", "pass", "browser-daemon responded to doctor", "")
        ...
        if not info.get("skill_synthetic"):
            if usable:
                ...
                add(
                    ...
                    "`browser-daemon serve --backend rdp`",
                )
        ...
        try:
            ...
            add(
                ...
                "reinstall browser-skill (`browser-skill install`) or check the "
