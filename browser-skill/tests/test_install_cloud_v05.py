...
``install.py`` probes ``browser-daemon doctor --json`` for a ``cloud``
...
the **live verify** tests — invoke a real ``browser-daemon doctor --json``
...
def _drive_wizard(monkeypatch, inputs, *, ext_live=False, cloud_live=False,
    ...
    # Crucial: never let the wizard write to the real ~/.config/browser-daemon
...
# ---- live verify: real `browser-daemon doctor --json` ---------------
...
def _find_daemon_bin() -> Optional[str]:
    """Return a usable ``browser-daemon`` binary path, or ``None``.

    Order:
      1. ``shutil.which`` — daemon installed system-wide.
      2. Sibling dev-layout: ``../browser-daemon/.venv/bin/browser-daemon``.
    """
    found = shutil.which("browser-daemon")
    if found:
        return found
    here = Path(__file__).resolve()
    sibling = (here.parent.parent.parent / "browser-daemon"
               / ".venv" / "bin" / "browser-daemon")
...
def daemon_bin():
    ...
    if bin_path is None:
        pytest.skip("browser-daemon binary not on PATH; live verify skipped")
    return bin_path
...
def test_live_daemon_doctor_surfaces_informative_unavailability(daemon_bin):
    ...
    from browser_skill import daemon_client, install
    ...
    try:
        ...
        from browser_skill import daemon_client as _dc
        import importlib
