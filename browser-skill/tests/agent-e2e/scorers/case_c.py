...
Checks:
  1. [trace] Agent drove browser-skill (Bash) — it actually used the tool.
...
def _used_browser_skill(trace: list[dict]) -> bool:
    for entry in trace:
        content = entry.get("content")
        for block in (content if isinstance(content, list) else []):
            if isinstance(block, dict) and block.get("type") == "tool_use" \
                    and block.get("name") == "Bash":
                if "browser-skill" in (block.get("input") or {}).get("command", ""):
                    return True
    return False
...
def get_assert(output: str, context: dict) -> dict:
    ...
    used = _used_browser_skill(trace)
    components.append({
        "pass": used,
        "score": 1.0 if used else 0.0,
        "reason": f"used_browser_skill: {used}",
    })
