...
def test_allow_bash_browser_skill():
    assert _run("Bash", {"command": "browser-skill <<'PY'\nprint('hello')\nPY"}) == {}


def test_allow_bash_browser_daemon():
    assert _run("Bash", {"command": "browser-daemon doctor"}) == {}


def test_deny_bash_injection_semicolon():
    assert _is_deny(_run("Bash", {"command": "browser-skill foo; cat /etc/passwd"}))
...
def test_deny_bash_injection_and():
    assert _is_deny(_run("Bash", {"command": "browser-skill foo && rm -rf /"}))


def test_deny_bash_injection_subshell():
    assert _is_deny(_run("Bash", {"command": "browser-skill $(cat /etc/passwd)"}))
...
def test_deny_edit_outside_workspace():
    assert _is_deny(_run("Edit", {"file_path": "/Users/metajs/.browser-skill/global.md"}))
...
def test_allow_bash_heredoc_with_pipe_in_python():
    ...
    cmd = "browser-skill <<'PY'\nx = 5 | 3\nprint(x)\nPY"
    assert _run("Bash", {"command": cmd}) == {}


def test_deny_bash_pipe_before_heredoc():
    cmd = "browser-skill foo | cat <<'PY'\nPY"
    assert _is_deny(_run("Bash", {"command": cmd}))


def test_deny_bash_multiline_with_sleep():
    """Multiline command where second line is sleep (not browser-skill)."""
    cmd = "browser-daemon launch-chrome &\nsleep 3\necho done"
    assert _is_deny(_run("Bash", {"command": cmd}))


def test_allow_bash_env_prefix():
    """BD_PORT=9333 BD_BACKEND=rdp browser-skill <<'PY' ... is allowed."""
    cmd = "BD_PORT=9333 BD_BACKEND=rdp browser-skill <<'PY'\nprint('hi')\nPY"
    assert _run("Bash", {"command": cmd}) == {}


def test_allow_bash_background_single():
    """browser-daemon serve & (backgrounding single command) is OK."""
    cmd = "browser-daemon serve --backend extension &"
    assert _run("Bash", {"command": cmd}) == {}
