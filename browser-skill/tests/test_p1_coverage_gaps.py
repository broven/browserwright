...
def test_infer_output_schema_dict_keys():
    from browser_skill.solidify.propose import _infer_output_schema
...
def test_cli_solidify_alias_dispatches_to_save(monkeypatch, capsys):
    """REVIEW.md F-16: ``browser-skill solidify`` should dispatch the
    same handler as ``browser-skill save``. Run with no args to get
