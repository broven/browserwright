...
def test_output_schema_simple_pass():
    from browser_skill.output_schema import validate
...
def test_output_schema_missing_required_fails():
    from browser_skill.output_schema import OutputSchemaError, validate
...
def test_project_level_overrides_bundled(tmp_path, monkeypatch):
    ...
    for name in list(sys.modules):
        if name.startswith("browser_skill"):
            del sys.modules[name]
    from browser_skill.discovery import find_task_path, rebuild_index
