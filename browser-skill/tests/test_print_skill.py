"""S7 gate: ``browser-skill --print-skill`` emits a skill doc generated from
...
printed doc is enumerated from ``browser_skill.EXPORTS`` at runtime, so it can
...
from browser_skill import skill_doc
...
def test_render_stamps_installed_version():
    ...
    hardcoded."""
    doc = skill_doc.render()
    assert browser_skill.__version__ in doc
...
def test_cli_print_skill_flag(capsys):
    """``browser-skill --print-skill`` wires the generator into dispatch and
    ...
    assert rc == 0
    assert browser_skill.__version__ in out
