...
from browser_skill.errors import (
    BrowserSkillError,
    CDPError,
    DaemonUnavailable,
    NeedsUserConfirm,
...
def test_base_error_accepts_fix():
    ...
    e = BrowserSkillError("something broke", fix="run `browser-skill doctor`")
    assert e.fix
    assert e.fix in str(e)  # surfaced in __str__


def test_explicit_fix_surfaces_in_serialize():
    e = DaemonUnavailable("connection refused", fix="start it: `browser-daemon serve`")
    d = serialize(e)
    assert d.get("fix")
