...
class CloudConfig:
    ...
    `browser_daemon.auth`. The kind-specific config dict is `auth` (raw
...
def load(
    ...
    # `"garbage"` is stored as-is and surfaces at `browser-daemon url` as
    ...
    if "BD_RDP_PORT" in e:
        ...
        try:
            ...
            raise UserError(
                ...
                f"{e['BD_PORT']!r}")
        # Stderr (NOT stdout — `browser-daemon url`'s stdout is the URL).
